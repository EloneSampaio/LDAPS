...
class adLDAP {
    ...
    * @var array
    */
    protected $domainControllers = array("172.16.5.4");
        ...
        if ($this->adminUsername !== NULL && $this->adminPassword !== NULL) {
            ...
            if (!$this->ldapBind) {
                if ($this->useSSL && !$this->useTLS) {
                    ...
                    throw new adLDAPException('Conexao com Active Directory falhou. Either the LDAPs connection failed or the login credentials are incorrect. AD said: ' . $this->getLastError());
                }
                else {
                    throw new adLDAPException('Conexao com Active Directory falhou. Check the login credentials and/or server details. AD said: ' . $this->getLastError());
                }
            }
        }
